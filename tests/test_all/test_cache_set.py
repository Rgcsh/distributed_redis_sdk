...
class TestCacheSet(TestBase):
    ...
    def test_normal(self, client):
        """ 测试 缓存1s
        """
        resp = client.get('api/cache_set/cache_set_a/1/1')
    ...
    def test_all_time(self, client):
        ...
        """
        # time_out=0
        resp = client.get('api/cache_set/cache_set_b/1/0')
        assert resp.data == b'true'
        # 睡眠后再获取
        time.sleep(0.2)
        ...
        # time_out=-1
        resp = client.get('api/cache_set/cache_set_c/1/0')
        assert resp.data == b'true'
        # 睡眠后再获取
        time.sleep(0.2)
        ...
        # time_out=-2
        resp = client.get('api/cache_set/cache_set_d/1/0')
        assert resp.data == b'true'
        # 睡眠后再获取
        time.sleep(0.2)
